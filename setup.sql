...
CREATE FUNCTION chat.GET_MESSAGES (p_message_count INT, p_user_id INT)
...
BEGIN
    ...
    SELECT * FROM chat.MESSAGES
        WHERE (sender_id = p_user_id OR recever_id = p_user_id)
        ORDER BY sent_at DESC LIMIT 25 OFFSET (p_message_count * 25);
END;
$$;
...
CREATE OR REPLACE FUNCTION chat.GET_FRIEND_DATA(p_user_id INT)
RETURNS TABLE (
    user_id				INT,
    username			VARCHAR(32),
...
BEGIN
    RETURN QUERY
    SELECT u2.user_id, u2.username, u2.user_created_at, f.request_accepted AS request_accepted,
        CASE
            WHEN f.friend1 = p_user_id THEN TRUE
            ELSE FALSE
        END AS initiated_by_me
    FROM chat.FRIENDS f
    JOIN chat.USERS u ON (u.user_id = f.friend1 OR u.user_id = f.friend2)
    JOIN chat.USERS u2 ON (
            (u2.user_id = f.friend1 AND u2.user_id != u.user_id) OR
            (u2.user_id = f.friend2 AND u2.user_id != u.user_id)
    )
